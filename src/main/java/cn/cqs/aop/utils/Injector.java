...
import cn.cqs.aop.annotation.BindView;
import cn.cqs.aop.annotation.OnClick;
import cn.cqs.aop.annotation.OnItemClick;
import cn.cqs.aop.annotation.OnItemLongClick;
import cn.cqs.aop.annotation.OnLongClick;
import cn.cqs.aop.annotation.Res;
...
public class Injector {
    ...
    private static void _injectIntent(ViewFinder viewFinder, Field field) throws IllegalAccessException, IllegalArgumentException, NoSuchMethodException {
        Autowired _autowired = field.getAnnotation(Autowired.class);
        if (null == _autowired) {
            return;
        }
        Object object = viewFinder.getObject();
        //获取参数字段名
        String name = _autowired.name();
